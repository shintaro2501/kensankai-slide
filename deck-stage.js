  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      if (/[?&]_snthumb=/.test(location.search)) {
        ...
        this.setAttribute('data-snthumb', '');
      }
      this._render();
      this._loadNotes();
    ...
    _flashOverlay() {
      ...
      if (!this._overlay || this._presenting || this.hasAttribute('data-fullscreen')) return;
    ...
    _fit() {
      ...
      // Add right breathing room when the rail is visible so the slide
      // doesn't butt against the browser frame. Zero when the rail is
      // hidden (T key, P key, noscale/no-rail) so those paths are unaffected.
      const padR = rw > 0 ? RAIL_PAD_RIGHT : 0;
      const vw = window.innerWidth - rw - padR;
    ...
    _makeThumb(slide) {
      ...
      const entry = { thumb, num, frame, slide, clone: null, wrapper: null, host: null, i: -1 };
    ...
    _materialize(entry) {
      ...
      const slideNumber = (entry.i >= 0 ? entry.i : (this._thumbs || []).indexOf(entry)) + 1;
      if (slideNumber > 0) {
      ...
      wrapper.className = 'thumb-scale-wrapper';
      wrapper.style.cssText = 'position:absolute;top:0;left:0;width:' + dw + 'px;height:' + dh + 'px;transform-origin:0 0;pointer-events:none;';
      const host = document.createElement('div');
      ...
      if (this._thumbScale) wrapper.style.transform = 'scale(' + this._thumbScale + ')';
    ...
    _refreshThumb(slide) {
      ...
      entry.host.remove();
      entry.host = entry.clone = entry.wrapper = entry.iframe = null;
    ...
    _scaleThumbs() {
      ...
      this._thumbs.forEach(({ wrapper }) => {
        if (wrapper) wrapper.style.transform = 'scale(' + this._thumbScale + ')';
      });
    }
